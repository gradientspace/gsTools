...
namespace gs
{
    public abstract class BaseNAryInputSOToolBuilder<T> : IToolBuilder where T : BaseNAryInputSOTool<T>
    {
    ...
    /// background-compute op to multiple DMeshSOs, where each input SO goes into
    /// the combine op separately
    /// </summary>
    public abstract class BaseNAryInputSOTool<T> : BaseToolCore, ITool where T : class, ITool
    {
        ...
        protected abstract BaseDMeshSourceOp edit_op_factory(IEnumerable<DMeshSourceOp> meshSourceOps);
        ...
        protected List<DMesh3> SceneMeshes;
        protected List<ConstantMeshSourceOp> MeshSourceOps;
